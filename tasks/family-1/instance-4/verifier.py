"""Verifier test suite for Family 1, Instance 4: process_orders (dependent).

Run with: pytest tasks/family-1/instance-4/verifier.py
Expects solution.py in the same directory, exposing process_orders.

This file is extracted verbatim from section 5 of
memory/tasks/family-1/instance-4.md, which is the design source of truth. It
...
def test_processed_preserves_input_order():
    orders = [order(id="c"), order(id="a"), order(id="b")]
    result = process_orders(orders, config())
    assert [e["id"] for e in result["processed"]] == ["c", "a", "b"]
...
# --- Components C and D: definitions and discount ----------------------------
...
def test_bulk_discount_at_threshold():
    # quantity equal to bulk_threshold qualifies as a bulk order
    result = process_orders(
        [order(customer="nobody", amount=100, quantity=10)], config())
    assert result["processed"][0]["final_amount"] == 90.0


def test_bulk_discount_below_threshold_not_applied():
    result = process_orders(
        [order(customer="nobody", amount=100, quantity=9)], config())
    assert result["processed"][0]["final_amount"] == 100.0
...
def test_mixed_validity_with_discounts():
    orders = [
